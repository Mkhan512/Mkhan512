...
def load_inventory(filename=None):
    ...
    file_to_load = filename if filename else inventory_file
...
def save_inventory(filename=None):
    file_to_save = filename if filename else inventory_file
    with open(file_to_save, mode='w', newline='') as file:
...
# Function to export inventory to a user-selected file
def export_inventory():
    file_path = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV files", "*.csv")])
    if file_path:
        save_inventory(file_path)
        messagebox.showinfo("Export", f"Inventory exported to {file_path}")

# Function to import inventory from a user-selected file
def import_inventory():
    file_path = filedialog.askopenfilename(filetypes=[("CSV files", "*.csv")])
    if file_path:
        load_inventory(file_path)
        messagebox.showinfo("Import", f"Inventory imported from {file_path}")
...
btn_export = tk.Button(window, text="Export Inventory", command=export_inventory)
btn_export.grid(row=6, column=0, padx=10, pady=10)

btn_import = tk.Button(window, text="Import Inventory", command=import_inventory)
btn_import.grid(row=6, column=1, padx=10, pady=10)
